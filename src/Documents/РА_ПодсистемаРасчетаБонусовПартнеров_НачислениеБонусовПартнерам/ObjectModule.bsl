...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.РА_ПодсистемаРасчетаБонусовПартнеров_БонусыПартнерам.Остатки(
		|			&Период,
...
Процедура ОбработатьНачислениеБонусовЗависящихОтПлана(ТаблицаНачисленийПоБонуснойПрограмме, ТаблицаОборотовПоДоговоруЗаПериод, БонуснаяПрограмма)
	...
	СтрокаНачисления.АналитикаУчетаБонуснойПрограммы = 
		Справочники.РА_ПодсистемаРасчетаБонусовПартнеров_КлючиАналитикиУчетаБонуснойПрограммы.ПодобратьКлючАналитикиУчетаБонуснойПрограммы(
...
Функция ПолучитьМассивДоговоровДляОтбораВыручки(Договор)
	...
	ОсновнойДоговор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор,"РА_ПодсистемаРасчетаБонусовПартнеров_ОсновнойДоговор");
...
Функция ИнициализироватьТаблицуНачисленийБонусов(Партнер, Договор, ПериодДействия, УидБонуснойПрограммы)
	...
	Если Не Договор.Пустая() Тогда
		...
		ЗначениеОсновногоДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "РА_ПодсистемаРасчетаБонусовПартнеров_ОсновнойДоговор");
	...
	Запрос.УстановитьПараметр("ПериодДействия", ПериодДействия);
    Запрос.УстановитьПараметр("Период", Новый Граница(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПериодДействия, "КонецПериода"), ВидГраницы.Исключая));
