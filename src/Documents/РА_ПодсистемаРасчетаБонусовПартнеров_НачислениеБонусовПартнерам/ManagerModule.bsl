...
Функция СформироватьПечатнуюФормуАкта(МассивОбъектов,ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Пока ДанныеПоШапке.Следующий() Цикл
		...
		Пока ВыборкаТабличнойЧасти.Следующий() Цикл 
			...
			СуммаНДСИтого = СуммаНДСИтого + ВыборкаТабличнойЧасти.СуммаНДС;
		...
		СтруктураПараметров.Вставить("ИтогоНДС", СуммаНДСИтого);
...
Функция ПолучитьДанныеДляПечатиАкта(МассивОбъектов) Экспорт
	...
	Запрос.Текст = 
		...
		|	СУММА(ВложенныйЗапрос.ПоказательЗакупки) КАК ПоказательЗакупки,
		|	ВложенныйЗапрос.ТипПремии КАК ТипПремии,
		|	ВложенныйЗапрос.ТипПоказателяПлана КАК ТипПоказателяПлана,
		|	ВложенныйЗапрос.ПроцентПремии КАК ПроцентПремии,
		|	СУММА(ВложенныйЗапрос.СуммаБонуса) КАК СуммаБонуса,
		|	СУММА(ВложенныйЗапрос.БонусируемыйОборот) КАК БонусируемыйОборот,
		|	ВложенныйЗапрос.ПоказательПлана КАК ПоказательПлана,
		|	ВложенныйЗапрос.ПроцентВыполненияПлана КАК ПроцентВыполненияПлана,
		|	СУММА(ВложенныйЗапрос.СуммаНДС) КАК СуммаНДС,
		|	ВложенныйЗапрос.ТекстНДС КАК ТекстНДС
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|		ВЫБОР
		|			КОГДА РА_НачислениеБонусовПартнерамНачисленияАвтоматическийРасчет.СуммаНДС > 0
		|				ТОГДА &ТекстОблагается
		|			ИНАЧЕ &ТекстНеОблагается
		|		КОНЕЦ КАК ТекстНДС
		|	ИЗ
		...
		|		ЗНАЧЕНИЕ(Перечисление.РА_ТипыПоказателяПлана.НетПлана),
		...
		|		РА_НачислениеБонусовПартнерамНачисленияАвтоматическийРасчет.БонусируемыйОборот,
		|		0,
		|		0,
		...
		|	ГДЕ
		|		РА_НачислениеБонусовПартнерамНачисленияАвтоматическийРасчет.Ссылка В(&МассивОбъектов)) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.Ссылка,
		|	ВложенныйЗапрос.ТекстНДС,
		|	ВложенныйЗапрос.ПроцентВыполненияПлана,
		|	ВложенныйЗапрос.ПоказательПлана,
		|	ВложенныйЗапрос.БонусируемыйОборот,
		|	ВложенныйЗапрос.ПроцентПремии,
		|	ВложенныйЗапрос.ТипПремии,
	...
	Запрос.УстановитьПараметр("ТекстОблагается", "НДС облагается");
	Запрос.УстановитьПараметр("ТекстНеОблагается", "НДС не облагается");
