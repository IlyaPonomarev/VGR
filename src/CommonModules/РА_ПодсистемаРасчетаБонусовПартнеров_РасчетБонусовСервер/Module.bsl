...
Процедура СформироватьДвиженияПоДействующимБонуснымПрограммам(Знач Договор, ДатаДокумента, Движения, СсылкаНаДокумент, ВнешнийВызов = Ложь)
	...
	Для Каждого ДействующаяПрограммаБонуса Из ТаблицаДействующихБонусныхПрограмм Цикл
		...
		МассивПодходящейНоменклатуры = ПолучитьМассивНоменклатурыДляРасчетаБонусов(ДействующаяПрограммаБонуса.БонуснаяПрограммаСсылка);
		...
		Если ДействующаяПрограммаБонуса.ТипПоказателяПлана <> ПредопределенноеЗначение(
			...
			Для Каждого СтрокаДвиженияВыручка Из ДвиженияПоВыручке Цикл
				...
				Если МассивПодходящейНоменклатуры.Найти(НоменклатураВыручки) <> Неопределено
					...
					Движение.БонуснаяПрограмма = ДействующаяПрограммаБонуса.БонуснаяПрограммаСсылка;
...
#Область РасчетБонусовЗаФорматОтгрузки
		...
		Если ДействующаяПрограммаБонуса.НастройкиПремийПоФорматамОтгрузки.Количество() Тогда
			...
			Для Каждого СтрокаДвиженияВыручка Из ДвиженияПоВыручке Цикл
				...
				Для Каждого СтрокаПоФорматуОтгрузки Из СтрокиПоФорматуОтгрузки Цикл
					...
					Если МассивПодходящейНоменклатуры.Найти(НоменклатураВыручки) <> Неопределено Тогда
						...
						Движение.АналитикаНастройкиПремий = ФорматОтгрузкиЗаказа;	
...
#Область РасчетБонусовЗаПредоставлениеСтатистическихДанных
		...
		Если ДействующаяПрограммаБонуса.ПроцентПремииЗаСтатистическиеДанные <> 0 Тогда
			...
			Для Каждого СтрокаДвиженияВыручка Из ДвиженияПоВыручке Цикл
				...
				Если МассивПодходящейНоменклатуры.Найти(НоменклатураВыручки) <> Неопределено
					...
					Движение.ПроцентПремии = ДействующаяПрограммаБонуса.ПроцентПремииЗаСтатистическиеДанные;
					Движение.СуммаВыручки = СтрокаДвиженияВыручка.СуммаВыручкиРегл;
					Движение.Сумма = СтрокаДвиженияВыручка.СуммаВыручкиРегл
						* ДействующаяПрограммаБонуса.ПроцентПремииЗаСтатистическиеДанные / 100;
...
#Область РасчетБонусовПоПлануЗакупокВидовОборудования
		...
		Если ДействующаяПрограммаБонуса.НастройкиПремийПоВидамОборудования.Количество() Тогда
			...
			Для Каждого СтрокаДвиженияВыручка Из ДвиженияПоВыручке Цикл
				...
				РеквизитыНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НоменклатураВыручки,
					"РА_ПодсистемаРасчетаБонусовПартнеров_ВидОборудованияДляНачисленияБонусов, РА_ВходитВБазуБонусовПриЗакупках");

				//@skip-check query-in-loop
				НастройкиПоВидамОборудования = ПолучитьТаблицуПремийПоВидамОборудованияПоБонуснойПрограмме(
					ДействующаяПрограммаБонуса.БонуснаяПрограммаСсылка);

				НастройкаПоВидуОборудования = НастройкиПоВидамОборудования.НайтиСтроки(
					Новый Структура("ВидОборудования",
					РеквизитыНоменклатуры.РА_ПодсистемаРасчетаБонусовПартнеров_ВидОборудованияДляНачисленияБонусов));

				Если МассивПодходящейНоменклатуры.Найти(НоменклатураВыручки) <> Неопределено
					И РеквизитыНоменклатуры.РА_ВходитВБазуБонусовПриЗакупках
					И НастройкаПоВидуОборудования.Количество() Тогда
					...
					Движение.АналитикаНастройкиПремий = РеквизитыНоменклатуры.РА_ПодсистемаРасчетаБонусовПартнеров_ВидОборудованияДляНачисленияБонусов;

					Движение.Номенклатура = НоменклатураВыручки;
					Движение.Количество = СтрокаДвиженияВыручка.Количество;
					Движение.ТипПремии = ПредопределенноеЗначение(
						"Перечисление.РА_ТипыПремийПартнерам.ПремияЗаВыполнениеПланаПоВидамОборудования");
					Движение.ПроцентПремии = НастройкаПоВидуОборудования[0].ПроцентПремии;
					Движение.СуммаВыручки = СтрокаДвиженияВыручка.СуммаВыручкиРегл;
					Движение.Сумма = СтрокаДвиженияВыручка.СуммаВыручкиРегл
						* НастройкаПоВидуОборудования[0].ПроцентПремии / 100;
				КонецЕсли;
...
// Получить действующие бонусные программы.
// 
// Параметры:
//  ОбъектНастройки - СправочникСсылка.Партнеры, СправочникСсылка.ДоговорыКонтрагентов - Объект настройки
//  Дата - Дата
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Получить действующие бонусные программы
Функция ПолучитьДействующиеБонусныеПрограммы(ОбъектНастройки, Дата)
	...
	|	РА_НастройкиНачисленияБонусовПартнерам.ПроцентПремииЗаСтатистическиеДанные,
	...
	|		Количество КАК Количество) КАК НастройкиПремийПоВидамОборудования
	|ИЗ
...
// Сформировать начисления бонусов клиентам.
Процедура СформироватьНачисленияБонусовКлиентам() Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	РА_НастройкиАвтоматическогоФормированияНачисленияБонусовПартнерам.ПериодичностьФормирования КАК
	|		ПериодичностьФормирования,
	|	РА_НастройкиАвтоматическогоФормированияНачисленияБонусовПартнерам.ОбновлятьРанееСозданныеДокументы КАК
	|		ОбновлятьРанееСозданныеДокументы,
	|	РА_НастройкиАвтоматическогоФормированияНачисленияБонусовПартнерам.УчетнаяЗаписьДляРассылки КАК
	|		УчетнаяЗаписьДляРассылки
	|ИЗ
	|	РегистрСведений.РА_НастройкиАвтоматическогоФормированияНачисленияБонусовПартнерам КАК
	|		РА_НастройкиАвтоматическогоФормированияНачисленияБонусовПартнерам";
...
// Получить массив номенклатуры для расчета бонусов по плану закупок.
// 
// Параметры:
//  БонуснаяПрограмма - СправочникСсылка.РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммы
// 
// Возвращаемое значение:
// Массив из СправочникСсылка.Производители 
Функция ПолучитьМассивНоменклатурыДляРасчетаБонусов(БонуснаяПрограмма) Экспорт
		
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммы.Производители.(
		|		Производитель),
		|	РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммы.ДополнительнаяНоменклатура.(
		|		Номенклатура)
		|ИЗ
		...
		|ГДЕ
		|	РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммы.Ссылка = &Ссылка";
	
	Запрос.УстановитьПараметр("Ссылка", БонуснаяПрограмма);
	
	НастройкиБонуснойПрограммы = Запрос.Выполнить().Выбрать();
	
	НастройкиБонуснойПрограммы.Следующий();
	
	МассивПроизводителей = НастройкиБонуснойПрограммы.Производители.Выгрузить().ВыгрузитьКолонку("Производитель");

	МассивДополнительнойНоменклатуры = НастройкиБонуснойПрограммы.ДополнительнаяНоменклатура.Выгрузить().ВыгрузитьКолонку(
				"Номенклатура");
...
// Получить таблицу премий по видам оборудования по бонусной программе.
// 
// Параметры:
//  БонуснаяПрограмма - СправочникСсылка.РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммы - Бонусная программа
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Получить таблицу премий по видам оборудования по бонусной программе
Функция ПолучитьТаблицуПремийПоВидамОборудованияПоБонуснойПрограмме(БонуснаяПрограмма) Экспорт
		
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммыНастройкиПремийПоВидамОборудования.ВидОборудования,
		|	РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммыНастройкиПремийПоВидамОборудования.ПроцентПремии,
		|	РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммыНастройкиПремийПоВидамОборудования.Количество
		|ИЗ
		|	Справочник.РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммы.НастройкиПремийПоВидамОборудования КАК
		|		РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммыНастройкиПремийПоВидамОборудования
		|ГДЕ
		|	РА_ПодсистемаРасчетаБонусовПартнеров_БонусныеПрограммыНастройкиПремийПоВидамОборудования.Ссылка = &Ссылка";
