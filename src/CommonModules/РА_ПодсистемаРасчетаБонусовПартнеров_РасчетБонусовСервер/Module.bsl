		...
		Если ДействующаяПрограммаБонуса.ТипПоказателяПлана <> ПредопределенноеЗначение("Перечисление.РА_ТипыПоказателяПлана.НетПлана") Тогда
			...
			Для Каждого СтрокаДвиженияВыручка Из ДвиженияПоВыручке Цикл
				...
				Если РеквизитыНоменклатуры.Производитель = ?(НЕ ЗначениеЗаполнено(ДействующаяПрограммаБонуса.Производитель), РеквизитыНоменклатуры.Производитель, ДействующаяПрограммаБонуса.Производитель) И РеквизитыНоменклатуры.РА_ВходитВБазуБонусовПриЗакупках Тогда
					...
					Движение.АналитикаУчетаБонуснойПрограммы = Справочники.РА_ПодсистемаРасчетаБонусовПартнеров_КлючиАналитикиУчетаБонуснойПрограммы.ПодобратьКлючАналитикиУчетаБонуснойПрограммы(
...
Функция ПолучитьДействующиеБонусныеПрограммы(ОбъектНастройки, Дата)
	...
	//	|ИЗ
	//	|	РегистрСведений.РА_ПодсистемаРасчетаБонусовПартнеров_НастройкиНачисленияБонусовПартнерам КАК РА_НастройкиНачисленияБонусовПартнерам
	//	|ГДЕ
...
Процедура СформироватьНачисленияБонусовКлиентам() Экспорт
	...
	|	КОНЕЦ КАК Договор,
	|	ЕСТЬNULL(РА_НачислениеБонусовПартнерам.Ссылка, ЗНАЧЕНИЕ(Документ.РА_ПодсистемаРасчетаБонусовПартнеров_НачислениеБонусовПартнерам.ПустаяСсылка)) КАК ДокументНачисление,
	...
	|ИЗ
	|	РегистрСведений.РА_ПодсистемаРасчетаБонусовПартнеров_НастройкиНачисленияБонусовПартнерам КАК РА_НастройкиНачисленияБонусовПартнерам
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РА_ПодсистемаРасчетаБонусовПартнеров_НачислениеБонусовПартнерам КАК РА_НачислениеБонусовПартнерам
	|		ПО (ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(РА_НастройкиНачисленияБонусовПартнерам.ОбъектНастройки) = ТИП(Справочник.Партнеры)
	|					ТОГДА РА_НастройкиНачисленияБонусовПартнерам.ОбъектНастройки
	|				ИНАЧЕ РА_НастройкиНачисленияБонусовПартнерам.ОбъектНастройки.Партнер
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЕСТЬNULL(РА_НачислениеБонусовПартнерам.Ссылка, ЗНАЧЕНИЕ(Документ.РА_ПодсистемаРасчетаБонусовПартнеров_НачислениеБонусовПартнерам.ПустаяСсылка)) = ЗНАЧЕНИЕ(Документ.РА_ПодсистемаРасчетаБонусовПартнеров_НачислениеБонусовПартнерам.ПустаяСсылка)
	|		КОНЕЦ
	|ИТОГИ ПО
	|	ОсновнойМенеджер,
	|	Контролирует";
